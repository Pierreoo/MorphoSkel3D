...
def point2sphere_distance_with_batch(p1, p2, distance):
    '''
    :param p1: size[B,N,3]
    :param p2: size[B,M,4]
    ...
    if distance == 'chamfer':
        dist_scalar = torch.sum(min_dist)
        return dist_scalar / p1_pnum
    elif distance == 'hausdorff':
        dist_scalar = torch.max(min_dist)
        return dist_scalar
    else:
        print('Unknown distance: %s. Exiting..' % (distance))


def sphere2point_distance_with_batch(p1, p2, distance):
    '''
    :param p1: size[B,N,4]
    :param p2: size[B,M,3]
